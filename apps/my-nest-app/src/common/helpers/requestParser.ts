 ...
 */
function operationParser(operation: string, value: string): FindOperator<any> | string | number {
 ...
 */
const makeFindOption = function (query: any, search_fields: string[]): FindManyOptions | undefined {
    ...
    let { q, attributes, include, limit, page, sort_by, ids, ...filters } = query;
    ...
    if (attributes) {
        ...
        selects.map((element: string) => {
            ...
            if (levelTwo) {
                findOption.select[levelOne] = findOption.select[levelOne] != true ? findOption.select[levelOne] : {};
                findOption.select[levelOne][levelTwo] = true;
                if (levelThree) {
                    findOption.select[levelOne][levelTwo] = findOption.select[levelOne][levelTwo] != true ? findOption.select[levelOne][levelTwo] : {};
    ...
    if (include) {
        ...
        let relations: any = [];
        if (!Array.isArray(include) && typeof include != 'object') include = include?.toString().split(',');
...
const makeFindOneOption = function (query: any): FindOneOptions | undefined {
    ...
    if (attributes) {
        ...
        findOption.select = {};
        let selects: any = [];
        if (!Array.isArray(attributes) && typeof attributes != 'object') attributes = attributes.toString().split(',');
        if (!Array.isArray(attributes) && typeof attributes == 'object') return;
        selects = attributes;
        selects.map((element: string) => {
            findOption.select = findOption.select ?? {};
